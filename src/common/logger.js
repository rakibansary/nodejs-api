...
const logger = winston.createLogger({
  transports,
  format: combine(colorize(), align(), timestamp(), basicFormat),
});
...
logger.logFullError = function (err) {
  ...
  logger.error(
    (err.signature ? `${err.signature} : ` : "") + util.inspect(err)
  );
  err.logged = true;
};
...
logger.decorateWithLogging = (serviceName, service) => {
  ...
  _.each(service, (method, name) => {
    ...
    service[name] = async (...args) => {
      ...
      logger.debug(
        `##input arguments, ${util.inspect(
          sanitizeObject(combineObject(params, args))
        )}`
      );
      try {
        ...
        if (result !== null && result !== undefined) {
          logger.debug(
            `##output arguments, ${util.inspect(sanitizeObject(result))}`
          );
        } else {
...
logger.decorateWithValidators = (serviceName, service) => {
  _.each(service, (method, name) => {
    ...
    service[name] = async function (...args) {
      ...
      const normalized = method.schema
        ? await joi.object(method.schema).validateAsync(value)
        : value;
      const newArgs = [];
