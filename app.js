  ...
  _.each(verbs, (def, verb) => {
    ...
    if (def.auth) {
      ...
      actions.push((req, res, next) => {
        if (!req.authUser) {
          return next(
            errors.newAuthError("Action is not allowed for invalid token")
          );
        }
        ...
        if (req.authUser.roles) {
          ...
          next(
            errors.newAuthError("You are not authorized to perform this action")
          );
        } else {
          next();
        }
    ...
    logger.info(
      `Endpoint discovered : [${access}] ${verb.toLocaleUpperCase()} /${
        config.API_VERSION
      }${url}`
    );
